"""
Module for the photometric data and parallaxes from the JWST late T and early Y
dwarf observations recorded in Beiler et al. 2024 (ApJ)
"""
...
def add_jwst_ydwarfs(input_path: str, database: h5py._hl.files.File) -> None:
    ...
    nirc_filts = ['F090W', 'F115W', 'F140M', 'F150W',
       'F150W2', 'F162M', 'F164N', 'F182M', 'F187N', 'F200W', 'F210M',
       'F212N', 'F250M', 'F277W', 'F300M', 'F322W2', 'F323N', 'F335M',
       'F356W', 'F360M', 'F405N', 'F410M', 'F430M', 'F444W', 'F460M',
       'F466N', 'F470N', 'F480M']
    
    for filt in nirc_filts:
        database.create_dataset(f"photometry/beiler2024/JWST/NIRCAM.{filt}", data=np.array(phot_data[f'{filt}'], dtype=float))
    
    miri_filts = ['F560W', 'F770W', 'F1065C', 'F1130W',
       'F1140C']
    
    for filt in miri_filts:
        database.create_dataset(f"photometry/beiler2024/JWST/MIRI.{filt}", data=np.array(phot_data[f'{filt}'], dtype=float))
