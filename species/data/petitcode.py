...
@typechecked
def add_petitcode_cool_clear(
    input_path: str,
    database: h5py._hl.files.File,
    wavel_range: Optional[Tuple[float, float]] = None,
    teff_range: Optional[Tuple[float, float]] = None,
    spec_res: Optional[float] = 1000.0,
) -> None:
    """
    ...
    if not os.path.isfile(data_file):
        print(
            "Downloading petitCODE cool model spectra (3.7 GB)...", end="", flush=True
        )
    ...
    for _, _, files in os.walk(data_folder):
        for filename in files:
            ...
            if wavel_range is None:
                ...
                flux.append(
                    data[:, 1] * 1e-9 * constants.LIGHT / (wavelength * 1e-6) ** 2
                )
                ...
                # (erg s-1 cm-2 Hz-1) -> (W m-2 um-1)
                data_flux = (
                    data[:, 1] * 1e-9 * constants.LIGHT / (data_wavel * 1e-6) ** 2
                )
    ...
    data_util.write_data(
        "petitcode-cool-clear", ["teff", "logg", "feh"], database, data_sorted
    )


@typechecked
def add_petitcode_cool_cloudy(
    input_path: str,
    database: h5py._hl.files.File,
    wavel_range: Optional[Tuple[float, float]] = None,
    teff_range: Optional[Tuple[float, float]] = None,
    ...
    data_util.write_data(
        "petitcode-cool-cloudy", ["teff", "logg", "feh", "fsed"], database, data_sorted
    )


@typechecked
def add_petitcode_hot_clear(
    input_path: str,
    database: h5py._hl.files.File,
    wavel_range: Optional[Tuple[float, float]] = None,
    teff_range: Optional[Tuple[float, float]] = None,
    ...
    if not os.path.isfile(data_file):
        print(
            "Downloading petitCODE hot clear model spectra (93 MB)...",
            end="",
            flush=True,
        )
        urllib.request.urlretrieve(url, data_file)
...
def add_petitcode_hot_cloudy(
    ...
    if not os.path.isfile(data_file):
        print(
            "Downloading petitCODE hot cloudy model spectra (276 MB)...",
            end="",
            flush=True,
        )
        urllib.request.urlretrieve(url, data_file)
    ...
    print(
        "Unpacking petitCODE hot cloudy model spectra (276 MB)...", end="", flush=True
    )
    tar = tarfile.open(data_file)
    tar.extractall(data_folder)
    tar.close()
